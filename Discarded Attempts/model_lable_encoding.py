...
drop = ["age","sex","city","province",
        ...
        "country_new","V1_new","confirmed_dt"]
...
rf = RandomForestRegressor(n_estimators=600,min_samples_split= 2, min_samples_leaf= 6 ,max_features= 'sqrt', max_depth= 70, bootstrap= True,oob_score=(True))
...
result = pd.DataFrame( rf.predict(test_select),columns=["duration"])
result = pd.merge(test_select_id,result,left_on = test_select_id.index,right_on = result.index)
result = result.drop(["key_0","index"],axis=1)
result = result.sort_values(by=['Id'])
result["Id"] = result["Id"].astype(int)
result.to_csv("kaggle_submit_4.txt",sep=",",index=False)
