...
def parse_symptoms(df):
    if re.search("fever",df.symptoms) and re.search("cough",df.symptoms) and re.search("diarrhea",df.symptoms):
        val = 8
    elif re.search("fever",df.symptoms) and re.search("cough",df.symptoms):
        val = 7      
    elif re.search("cough",df.symptoms) and re.search("diarrhea",df.symptoms):
        val = 6   
    elif re.search("fever",df.symptoms) and re.search("diarrhea",df.symptoms):
        val = 5 
    elif re.search("cough",df.symptoms):
        val = 4
    elif re.search("fever",df.symptoms):
        val = 3
    elif re.search("diarrhea",df.symptoms):
        val = 2
    elif df.symptoms != "":
        val = 1
    else:
...
full_table["confirmed"].fillna("01.02.2020")
...
mean_age = full_table["age_new"].median(skipna=True)
...
full_table['cp'] = full_table.city + full_table.province
full_table['cc'] = full_table.province + full_table.country
...
full_table_onehot = pd.get_dummies(full_table, columns=["cp","cc", "country_cat",
                                    "V1_cat", "confirmed_cat","maj_symptoms"], 
                                    prefix=["cp","cc", "country_cat",
                                    "V1_cat", "confirmed_cat","maj_symptoms"],drop_first=True)
...
drop = ["age","sex","city","province",
        ...
        "city_province","province_country" ,"confirmed_dt","sex_cat",
        "city_cat","province_cat","outcome_cat"]
